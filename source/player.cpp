...
void Player::AcceptUserInput(int BGid) {
  ...
  if(AnimLoop == 0) {
    ...
    if     (keysHeld() & KEY_RIGHT) { Input.x =  1; InputDirection = DIR_RIGHT; }
    else if(keysHeld() & KEY_DOWN)  { Input.y =  1; InputDirection = DIR_DOWN;  }
    else if(keysHeld() & KEY_LEFT)  { Input.x = -1; InputDirection = DIR_LEFT;  }
    else if(keysHeld() & KEY_UP)    { Input.y = -1; InputDirection = DIR_UP;    }
